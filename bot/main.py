...
async def tortilla(ctx):
    ...
    embed.set_author(name=ctx.author.display_name,
                     url="https://twitter.com/Dumbbitchcat", icon_url=ctx.author.avatar_url)
    ...
    embed.add_field(
        name="Stuffify", value="Creates and sends an 'I'm Stuff' macro meme with custom text.", inline=True)
    embed.add_field(
        name="Adit", value="Command that sends a DM and/or @s the member 'Adit'", inline=True)
    embed.add_field(
        name="Adit.int", value="Posts a picture of the member 'Adit' inting or feeding. Gallery for said pictures is available at ___.", inline=True)
    embed.add_field(
        name="Team.int", value="Posts a picture of an entire team inting or feeding. Gallery for said pictures is available at ___.", inline=True)
    embed.add_field(
        name="4k", value="Posts an extremely sus picture. Gallery for said pictures is available at ___.", inline=True)
    ...
    embed.add_field(
        name="Susfinder", value="Replies to the command message with a message containing only the characters 's', 'u', and 's' in order.", inline=True)
